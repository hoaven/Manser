...
<%
	String realuser = (String) session.getAttribute("realuser");
	if (realuser == null || realuser == "") {
		request.setAttribute("message",
				"<script type='text/javascript'>alert('对不起，您还没有登录或者登录信息失效！');</script>");
...
<a class="right carousel-control" href="#carousel1" data-slide="next">
    <span class="glyphicon glyphicon-chevron-right"></span>
								...
								<li onmouseover="displaySubMenu(this)"
									...
									<ul>
										<c:if test="${sq.value!=null}">
											<c:forEach items="${sq.value}" var="wg">
												<li><a
													href="${pageContext.request.contextPath}/servlet/LimitServlet?value1=胜阳港片区管理委员会&value2=${sq.key}&obj=${wg}">${wg}</a>
												</li>
											</c:forEach>
										</c:if>
									</ul>
		...
		<c:if test="${isDisplay eq 'yes'}">
			<c:if test="${c ne '/statistics/records/Showrecords.jsp'}">
				<div class="container">
					<div class="row">
						<div class="col-md-6">
							<form class="form-inline"
								...
								<div class="form-group">
									<label><span>关键字查询:</span> </label> <select name="selectkey"
										class="form-control">
						...
						<div class="col-md-6">
							<ul id="mytab">
                                ...
                                -->
								<li><a
									href="${pageContext.request.contextPath}/servlet/AdminServlet?operation=deletemany&type=${type}">删除</a>
								</li>
								<li><a
									href="${pageContext.request.contextPath}/servlet/AdminServlet?operation=addone&type=${type}">增加</a>
								</li>
							</ul>
						</div>
					</div>
